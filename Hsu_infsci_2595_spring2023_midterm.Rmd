...
head(summary_post_df_focus_from_vague, 5)
```
...
summary_post_df_focus_from_vague %>%
  ...
  geom_linerange(ymin = summary_post_df_focus_from_vague$post_q05, ymax = summary_post_df_focus_from_vague$post_q95) +
  geom_point(y = summary_post_df_focus_from_vague$post_avg) +
  geom_point(y = summary_post_df_focus_from_vague$num_events/ summary_post_df_focus_from_vague$num_trials, color = "blue") +
  labs(y = "Event Probability") +
  labs(x = "Player ID")
```

What do you think?  
From this picture above, there's no MLEs outside posterior uncertainty interval, can check the blue dots and their lines.
Some players (such as ID:20, 232, 636 and so on) has same mean to their MLE.
...
summary_post_df_focus_from_vague %>% 
  ...
  geom_point(mapping = aes(y = post_avg,
                           group = player_id),position = position_dodge(0.2)) +
  geom_point(mapping = aes(y = num_events / num_trials,
                           group = player_id), color = "blue",position = position_dodge(0.2)) +
  labs(y = "Event Probability") +
  labs(x = "Player ID")
```
...
summary_post_df_all_from_vague <- post_df_all_from_vague %>%
  mutate(post_avg = anew / (anew + bnew),
         post_q05 = qbeta(0.05, anew, bnew),
         post_q95 = qbeta(0.95, anew, bnew),
         post_90 = post_q95 - post_q05
  )
head(summary_post_df_all_from_vague, 5 )
```
...
summary_post_df_all_from_vague %>%
  ggplot(aes(x = anew/ (anew + bnew), y = post_avg, color = "num_trials")) +
  geom_point() +
...
summary_post_df_all_from_vague %>%
  ggplot() +
  geom_point(aes(x = post_90, y = num_trials, color = "num_trials")) + 
  xlab("90% Uncertainty Interval")
```
...
$$
\sum_{j=1}^{J}(N_j) - \sum_{j=1}^{J}(m_j) = \sum_{j=1}^{J}(N_j) - \sum_{j=1}^{J}(m_j) = JN - Jm = J(N - m)
$$
$$
\log(p(\mu \mid (m, N) ) ) \propto  Jm \times \log(\mu) + J(N - m) \times \log(1 - \mu) + (a - 1)\log(\mu) + (b - 1)\log(1 - \mu)
$$
### 2c)
...
Add as many equation blocks as you feel are necessary to show the steps to derive the answer. 
$$
Jm \times \log(\mu) + (a - 1)\log(\mu) = (Jm + a - 1) \times \log(\mu)
$$
$$
a_{pool} = Jm + a
$$
$$
J(N - m) \times \log(1 - \mu) + (b - 1)\log(1 - \mu) = (J(N - m) + b -1) \times \log(1 - \mu)
$$
...
#J = nrow(df_focus), m = mean(df_all$num_events)
post_df_focus_pooled <- df_focus %>% 
  mutate(anew = nrow(df_all) * (mean(df_all$num_events)) + 0.5,
         bnew = nrow(df_all)*((mean(df_all$num_trials) - mean(df_all$num_events)) + 0.5))
head(post_df_focus_pooled, 5)
...
head(summary_post_df_focus_pooled, 5)
...
summary_post_df_focus_pooled %>%
  ...
  geom_linerange(mapping = aes(ymin = post_q05, ymax = post_q95)) +
  geom_point(mapping = aes(y = post_avg)) +
  geom_point(mapping = aes(y = num_events/ num_trials), color = "blue") #MLE
...
log_ab_res_01 <- optim(
  ...
  method = "BFGS",
  hessian=TRUE,
  control=list(fnscale=-1, maxit=2500)
)
```
...
df_24 %>%
  ggplot(mapping = aes(x = num_events / num_trials)) +
  geom_histogram(mapping = aes(y = stat(density)), fill = "purple") +
  geom_line(data = prior_for_viz, mapping = aes(x = x, y = beta_pdf), color = "green") +
  xlab("Density")
```
...
Add as many code chunks as you feel are necessary.  
```{r}
mean_24 = mean(df_24$num_events/ df_24$num_trials) #Mean
q05 = qbeta(0.05, shape1 = ab_emp_bayes[1], shape2 = ab_emp_bayes[2]) #0.05 Quantile
q95 = qbeta(0.95, shape1 = ab_emp_bayes[1], shape2 = ab_emp_bayes[2]) #0.95 Quantile
interval_24 = q95 - q05 #middle 90% uncertainty interval
c(mean_24, q05, q95, interval_24)
```

```{r}
mean_24 = mean(df_24$num_events/ df_24$num_trials) #Mean
q05_update = qbeta(0.05, shape1 = 13, shape2 = 8) #0.05 Quantile
q95_update = qbeta(0.95, shape1 = 13, shape2 = 8) #0.95 Quantile
interval_24_update = q95_update - q05_update #middle 90% uncertainty interval
c(mean_24, q05_update, q95_update, interval_24_update)
```
...
summary_post_df_focus_empbayes %>%
  ...
  geom_linerange(mapping = aes(ymin = post_q05, ymax = post_q95)) +
  geom_point(mapping = aes( y = post_avg)) +
  geom_point(mapping = aes( y =num_events/ num_trials), color = "purple")
```
...
summary_post_df_focus_empbayes %>%
  ...
  geom_point(mapping = aes( y =num_events/ num_trials, group = player_id), color = "purple")
```
### 4e)
...
```{r, solution_4h}
summary_post_df_all_empbayes %>%
  ggplot(mapping = aes(x = num_trials, y = interval)) + 
  geom_point(mapping = aes(color = num_trials)) +
  geom_abline(slope = 1, intercept=0)
```
...
compare_post_summaries %>%
  ggplot(mapping = aes(x= num_trials, y = post_90)) +
...
Add as many code chunks as you feel are necessary.  
They have large numer of trials than others. 
```{r, solution_5b}
post_player_eval %>%
...
```{r, solution_5f}
post_player_eval %>%
  ggplot(mapping = aes(x = post_avg, y = interval)) + 
  geom_point(mapping = aes(color = 'num_trials'))
```
