...
### your code here
library(ggplot2)
ggplot(data = df_low_high) + geom_point(aes(x, y), size = 3)
```
...
mod1 <- modelr::rmse(fit_lm_1, df_low_high)
mod2 <- modelr::rmse(fit_lm_2, df_low_high)
mod3 <- modelr::rmse(fit_lm_3, df_low_high)
mod4 <- modelr::rmse(fit_lm_4, df_low_high)
mod5 <- modelr::rmse(fit_lm_5, df_low_high)
mod6 <- modelr::rmse(fit_lm_6, df_low_high)
mod7 <- modelr::rmse(fit_lm_7, df_low_high)
mod8 <- modelr::rmse(fit_lm_8, df_low_high)
mod9 <- modelr::rmse(fit_lm_9, df_low_high)
df = data.frame(Model = c(1,2,3,4,5,6,7,8,9), RSME = c(mod1,mod2,mod3,mod4,mod5,mod6,mod7,mod8,mod9))
df
ggplot(data = df, mapping = aes(x = Model, y = RSME)) +
...
```{r, solution_01d}
### add as many code chunks as necessary
modle1 = modelr::rsquare(fit_lm_1, df_low_high)
modle2 = modelr::rsquare(fit_lm_2, df_low_high)
modle3 = modelr::rsquare(fit_lm_3, df_low_high) 
modle4 = modelr::rsquare(fit_lm_4, df_low_high)
modle5 = modelr::rsquare(fit_lm_5, df_low_high)
modle6 = modelr::rsquare(fit_lm_6, df_low_high)
modle7 = modelr::rsquare(fit_lm_7, df_low_high)
modle8 = modelr::rsquare(fit_lm_8, df_low_high)
modle9 = modelr::rsquare(fit_lm_9, df_low_high)

df1 = data.frame(Model = c(1,2,3,4,5,6,7,8,9), Rsquare = c(modle1,modle2,modle3,modle4,modle5,modle6,modle7,modle8,modle9))
df
ggplot(data = df1, mapping = aes(x = Model, y = Rsquare)) +
  geom_point() + geom_line()
...
What do you think?  
In my opinion, if just according R-square and RMSE, modle-9 showes the best model in these models, since not matter in Rsquare and in 
RMSE, they shows the lowest RMSE and the highest R-square. However, in you class, you said "The polynomial degree represents complexity." Model-9 may have overfit issue, which means the model is sensitive to data, but our main goal is finding out a non-sensitive model to be our ideal model. We need to use 5-folds or 10-folds cross-validation.
...
What do you think?  
1. model-9 has very uncertain coefficients, its ranges(lines) are big, so its uncertainty increased, too. Also, its extreme coefficients from -15 to 10. Model-2 is the best models within nine models because its extreme coefficients are not obvious and uncertainty are low and model-2 a easy model.

2. Yes, I think the coefficient estimates similar for those coefficients present in at least 2 of the models visualized in the previous figure. From lm(), model-2 doesn't have I(x ^ 3) and I(I ^ 4), and it doesn't show in Coefficient plot. Accordingly, the coefficient estimates similar to the coeffients.

3. In my opinion, no, uncertainty doesn't associate with the coefficients, because we can see I(x ^ 9) and I(x ^ 8), their uncertainty are not not large, can tell uncertainty through the line. The lines of I(x ^ 9) and I(x ^ 8) are short.
...
mod1 = lm(y ~x, data = df_low_high)
viz_trend_1 <- predict(mod1, input_viz)
```

```{r, solution_02b_b, eval=TRUE}
### quadratic relationship (degree 2) predictions
mod2 = lm(y ~x + I(x ^ 2), data = df_low_high)
viz_trend_2 <- predict(mod2, input_viz)

```

```{r, solution_02b_c, eval=TRUE}
### cubic relationship (degree 3) predictions
mod3 = lm(y ~x + I(x ^ 2) + I(x ^ 3), data = df_low_high)
viz_trend_3 <- predict(mod3, input_viz)
```

```{r, solution_02b_d, eval=TRUE}
### degree 4
mod4 = lm(y ~x + I(x ^ 2) + I(x ^ 3) + I(x ^ 4), data = df_low_high)
viz_trend_4 <- predict(mod4, input_viz)
```

```{r, solution_02b_e, eval=TRUE}
### degree 5
mod5 = lm(y ~x + I(x ^ 2) + I(x ^ 3) + I(x ^ 4) + I(x ^ 5), data = df_low_high)
viz_trend_5 <- predict(mod5, input_viz)
```

```{r, solution_02b_f, eval=TRUE}
### degree 6
mod6 = lm(y ~x + I(x ^ 2) + I(x ^ 3) + I(x ^ 4) + I(x ^ 5) + I(x ^ 6), data = df_low_high)
viz_trend_6 <- predict(mod6, input_viz)
```

```{r, solution_02b_g, eval=TRUE}
### degree 7
mod7 = lm(y ~x + I(x ^ 2) + I(x ^ 3) + I(x ^ 4) + I(x ^ 5) + I(x ^ 6) + I(x ^ 7), data = df_low_high)
viz_trend_7 <- predict(mod7, input_viz)
```

```{r, solution_02b_h, eval=TRUE}
### degree 8
mod8 = lm(y ~x + I(x ^ 2) + I(x ^ 3) + I(x ^ 4) + I(x ^ 5) + I(x ^ 6) + I(x ^ 7) + I(x ^ 8), data = df_low_high)
viz_trend_8 <- predict(mod8, input_viz)
```

```{r, solution_02b_i, eval=TRUE}
### degree 9
mod9 = lm(y ~x + I(x ^ 2) + I(x ^ 3) + I(x ^ 4) + I(x ^ 5) + I(x ^ 6) + I(x ^ 7) + I(x ^ 8) + I(x ^ 9), data = df_low_high)
viz_trend_9 <- predict(mod9, input_viz)
```
...
df_viz_all <- dplyr::bind_rows(df_viz_1, df_viz_2, df_viz_3, df_viz_4, df_viz_5, df_viz_6, df_viz_7, df_viz_8, df_viz_9)
df_viz_all
```
...
What do you think?  
Predictive trends associated with the best performing model would also depend on the specific task and dataset. However, in general, the best performing model should have a high level of accuracy, meaning that it makes correct predictions a high percentage of the time. Additionally, the model's precision and recall should be high, indicating that it correctly identifies positive instances and does not incorrectly identify negative instances.

Yes, the trends seem sonsistent with the training set.
...
set.seed(202214)
id_train <- sample(1:nrow(df_low_high) , size = 0.8 * nrow(df_low_high))
...
### add your code here
library(ggplot2)
ggplot(data = df_holdout) + geom_point(aes(x, y), size = 3)
```
...
fit_test_1 = modelr::rmse(fit_lm_1, df_holdout)
fit_test_2 = modelr::rmse(fit_lm_2, df_holdout)
fit_test_3 = modelr::rmse(fit_lm_3, df_holdout)
fit_test_4 = modelr::rmse(fit_lm_4, df_holdout)
fit_test_5 = modelr::rmse(fit_lm_5, df_holdout)
fit_test_6 = modelr::rmse(fit_lm_6, df_holdout)
fit_test_7 = modelr::rmse(fit_lm_7, df_holdout)
fit_test_8 = modelr::rmse(fit_lm_8, df_holdout)
fit_test_9 = modelr::rmse(fit_lm_9, df_holdout)
```
...
What do you think?  
1. I'll say model2 and model3 are all good, but I'll pick model2, since it's easy to read and understand.
2. Yes, we'll train training data first. Then fit test data to the model which is training by training data.
3. 80/20 training/ test split is the ideal rank to model. 
...
```{r, solution_03f}
### add your code here
coefplot::multiplot(fit_lm_1, fit_lm_2,fit_lm_3,fit_lm_4,fit_lm_5)
```

What do you think?  
In this plot, Model1 and Model2 seems better models.
...
```{r, solution_03g}
### add your code here
coefplot::multiplot(fit_train_1, fit_train_2,fit_train_3,fit_train_4,fit_train_5)
```
...
How many times will an individual model be trained and tested?  5 times
...
Since RMSE has the same unit as as the response, it's easier to explain to someone else.
...
dotplot(lowhigh_results, metric = "MAE")
dotplot(lowhigh_results, metric = "RMSE")
dotplot(lowhigh_results, metric = "Rsquared")
dotplot(lowhigh_results)
```
...
```{r, solution_04f}
### your code here
coefplot::multiplot(mod_lowhigh_3, mod_lowhigh_2, mod_lowhigh_5)
```
...
### your code here
library(ggplot2)
ggplot(df_low_low, aes(x = x, y =y)) + geom_point()
```

How does it compare?  
Dots in Problem 1a) are patchy, but Problem 1a) focus like a line. We can even identify it as a trend with parabola.
...
summary(lowlow_results)
```


```{r, solution_05c_b}
### visualize the dotplot for the performance metric
dotplot(lowlow_results)
dotplot(lowlow_results, metric = "Rsquared")
```

Which model is the best?  model-5 in Rsquared is the best.
...
What do you think?  
Yes, I think model2 is the best model in low and high models is because the feature are not too much, which cause overfit situation to model. We can know model-9 overfits in high and also in low.
...
### your code here
library(ggplot2)
ggplot(df_high_high, aes(x = x, y = y)) + geom_point()
```
...
```{r, solution_06c_a}
### print the resampling summary for the performance metric
  summary(highhigh_results)
```


```{r, solution_06c_b}
### visualize the dotplot for the performance metric
dotplot(highhigh_results)
dotplot(highhigh_results, metric = "Rsquared")
...
What do you think?  
In my opinion, model-2 are the best, but it still affect by sample size case. If we have more sample, we could have more precise results to identify which model is the best.
...
If we have more samples, complex model such as model-9 influence by samples more. We'll find model-9 fits to those data precise.
